@php
    $case = $adata['case'];
    $row2 =  $case->collectivesInvForConcilationEmp; //លិខិតអញ្ជើញតំណាងកម្មករមកផ្សះផ្សារ
    $row3 = $case->collectivesInvForConcilationCom; //លិខិតអញ្ជើញតំណាងក្រុមហ៊ុនមកផ្សះផ្សារ
    $caseYear = date2Display($case->case_date, "Y");
    $letter = $adata['letter'];
    $letterPair = $adata['letterPair'];

    if($letter->invitation_type_id == 9){ //លិខិតអញ្ជើញតំណាងកម្មករមកផ្សះផ្សារ
        $label1 = "ទាញយកលិខិតអញ្ជើញតំណាងកម្មករនិយោជិត";
        $label2 = "ទាញយកលិខិតអញ្ជើញតំណាងសហគ្រាស គ្រឹះស្ថាន";
        $invType1 = "លេខលិខិតអញ្ជើញតំណាងកម្មករនិយោជិត";
        $invType2 = "លេខលិខិតអញ្ជើញតំណាងសហគ្រាស គ្រឹះស្ថាន";

    }elseif($letter->invitation_type_id == 10){ //លិខិតអញ្ជើញតំណាងក្រុមហ៊ុនមកផ្សះផ្សារ
        $label1 = "ទាញយកលិខិតអញ្ជើញតំណាងសហគ្រាស គ្រឹះស្ថាន";
        $label2 = "ទាញយកលិខិតអញ្ជើញតំណាងកម្មករនិយោជិត";
        $invType1 = "លេខលិខិតអញ្ជើញតំណាងសហគ្រាស គ្រឹះស្ថាន";
        $invType2 = "លេខលិខិតអញ្ជើញតំណាងកម្មករនិយោជិត";
    }
@endphp
{{--{{ dd($letter->invitation_type_id) }}--}}
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-body row mt-2">
                        ...
                        @if(count($case->log6) == 0)
                            <div class="form-group col-sm-4">
                                <form name="frmDelete" action = "{{ url('collectives_invitation'.'/'.$letter->id."_".$adata['id_pair']) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    {{--                                ."_".$adata['id_pair']--}}
                    ...
                    <form name="formUpdateBothInv" action="{{ url('collectives_invitation/update_both') }}" method="POST" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
...
{{--                                    </div>--}}
                                ...
                                <div id="" class="row">
                                    @foreach($letter->nextTime as $next)
                                        ...
                                        <div class="form-group col-sm-1">
                                            @php
                                                $deleteUrl = url('collectives_invitation/delete/next/'.$caseYear.'_'.$next->id);
