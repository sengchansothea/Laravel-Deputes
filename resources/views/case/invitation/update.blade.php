@php
    ...
    $arrOfficerIDs = getCaseOfficerIDs($row->case->id);
@endphp
                ...
                <div class="card">
                    <div class="card-body row mt-2">
                        ...
                        <div class="form-group col-sm-4">
                            <a class="btn btn-info custom form-control mb-1 fw-bold" href="{{ url('export/word/invitation/'.$row->id) }}" title="Download" target="_blank">ទាញយកលិខិតសាកសួរ</a>
                        </div>
                        <div class="form-group col-sm-4">
                            <a class="btn btn-info custom form-control mb-1 fw-bold" href="{{ url('export/word/invitation/'.$row->id."/2") }}" title="Download" target="_blank">ទាញយកលិខិតដោះស្រាយ</a>
                        </div>
{{--                        <div class="form-group col-sm-4"></div>--}}
                        @if($row->invitation_type_id == 1)
                            @if(count($row->case->log34) == 0)
                                <div class="form-group col-sm-4">
                                    <form name="frmDelete" action = "{{ url('invitations'.'/'.$id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        {{--                                ."_".$adata['id_pair']--}}
                            ...
                            @endif
                        @elseif($row->invitation_type_id == 2)
                            @if(count($row->case->log5) == 0)
                                @if($chkAllowAccess || in_array($userOfficerID, $arrOfficerIDs) || auth()->user()->id == $row->case->user_created)
                                <div class="form-group col-sm-4">
                                    <form name="frmDelete" action = "{{ url('invitations'.'/'.$id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                    ...
                    <form name="formCreateCase" action="{{ url('invitations/'.$row->id) }}" method="POST" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        ...
                        <input type="hidden" name="invitation_type" value="{{ $row->invitation_type }}" >
                        <div class="card-body text-hanuman-17">
                            <div class="card-block row">
                                <div class="">
                                    <div class="row">
                                        <div class="form-group col-sm-6 mt-3">
                                            <label class="text-purple text-hanuman-24 mb-1 required">លេខលិខិតអញ្ជើញ</label>
                                    ...
                                    @if($row->invitationType->employee_or_company == 1)
                                        <div class="row">
                                            ...
                                            @if(!empty($row->caseDisputant->phone_number2))
                                                ...
                                                <div class="form-group col-sm-4 mt-3">
                                                    <label class="fw-bold mb-1">មុខងារ</label>
                                            ...
                                            @else
                                                ...
                                                <div class="form-group col-sm-3 mt-3">
                                                    <labelf class="fw-bold mb-1">លេខទូរស័ព្ទ</labelf>
                                                    <input type="text" value="{{ $row->caseDisputant->phone_number }}" class="form-control" disabled />
                                                </div>
                                            @endif
                                        </div>
...
{{--                                            <input type="text" name="reason" value="{{ $row->case->case_objective_des }}" class="form-control" disabled />--}}
                                            {!! showTextarea("reason", $row->case->case_objective_des, 4, "readonly") !!}
                                        </div>
