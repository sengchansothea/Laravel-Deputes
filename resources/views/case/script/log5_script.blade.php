...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#represent_company_nationality').on('change', function() {
            ...
            $('#represent_company_pob_country_id').val(selectedValue).trigger(
                'change'); // Update and trigger change event
        });
        ...
        // --- Checkbox show/hide log5 Represent Company ---
        ...
        $('#business_activity2').on('change', function() {
            ...
            // AJAX request
            $.ajax({
                url: "{{ url('ajaxGetBusinessActivity3') }}/" + business_activity1 + "/" +
                    business_activity2,
                type: 'get',
                data: {
                    "_token": "{{ csrf_token() }}"
        ...
        $('#business_activity3').on('change', function() {
            ...
            // AJAX request
            $.ajax({
                url: "{{ url('ajaxGetBusinessActivity4') }}/" + business_activity1 + "/" +
                    business_activity2 + "/" + business_activity3,
                type: 'get',
                data: {
                    "_token": "{{ csrf_token() }}"
                },
        ...
        eventChangeAddress("", "log5_head_province_id", "log5_head_district_id", "log5_head_commune_id",
        ...
        $("#represent_company_phone_number, #represent_company_phone2_number, #log5_head_phone, #log5_company_phone_number, #log5_total_employee, #log5_total_employee_female, #log5_union1_number")
        ...
        $("#open_date, #registration_date, #meeting_date, #meeting_stime, #meeting_etime").keydown(function(
            event) {
            return false;
        });
        ...
        $("#find_employee_autocomplete").autocomplete({
            source: function(request, response) {
                ...
                $.ajax({
                    url: "{{ url('/find_employee_autocomplete') }}" + "/" + $("#company_id")
                        .val(),
                    dataType: "json",
                    data: {
                        query: request.term
            ...
            select: function(event, ui) {
                ...
                $.ajax({
                    url: "{{ url('/autocomplete/get_employee_detail') }}" + "/" + $(
                        "#company_id").val(),
                    dataType: "json",
                    data: {
                        name: ui.item.value
                    ...
                    success: function(data) {
                        ...
                        $("#name" + counter_disputant).val(data[0]
                            .name); // Replace 'details' with the actual field name
                        var gender = 1;
                        if (data[0].gender == "ស្រី")
                            gender = 2;
                        $("#gender" + counter_disputant).select2().val(gender).trigger(
                            "change");
                        ...
                        $("#occupation" + counter_disputant).val(data[0].occupation)
                            .trigger("change");

                        $("#nationality" + counter_disputant).select2().val(data[0]
                            .nationality).trigger("change");
                        $("#id_number" + counter_disputant).val(data[0].id_number);
                        $("#phone_number" + counter_disputant).val(data[0]
                            .phone_number);
                        ...
                        $("#pob_province_id" + counter_disputant).select2().val(
                            pob_provinc_id).trigger("change");
                        ...
                        $("#province" + counter_disputant).select2().val(province)
                            .trigger("change");
