<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(".number_only_d").keypress(function(event){
            if (!( (event.charCode >= 48 && event.charCode <= 57) || event.charCode == 46)){ // 0-9 and .
                event.preventDefault();
                return false;
            }
        });
                ...
                select: function(event, ui) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            // Set province and trigger change to load districts
                            $("#"+addr_province+counter).val(data.province_id).trigger("change");
                            
                            // Use a recursive function to wait for districts to load
                            function loadDistrictWithRetry(retryCount = 0) {
                                if (retryCount > 20) return; // Stop after 20 retries (10 seconds max)
                                
                                setTimeout(function() {
                                    var districtSelect = $("#"+addr_district+counter);
                                    
                                    // Check if district has options (more than just the placeholder)
                                    if (districtSelect.find('option').length > 1) {
                                        // Districts are loaded, set the value
                                        districtSelect.val(data.district_id).trigger("change");
                                        
                                        // Now load commune
                                        loadCommuneWithRetry(0);
                                    } else {
                                        // Districts not loaded yet, retry
                                        loadDistrictWithRetry(retryCount + 1);
                                    }
                                }, 500); // Check every 500ms
                            }
                            
                            function loadCommuneWithRetry(retryCount = 0) {
                                if (retryCount > 20) return;
                                
                                setTimeout(function() {
                                    var communeSelect = $("#"+addr_commune+counter);
                                    
                                    if (communeSelect.find('option').length > 1) {
                                        communeSelect.val(data.commune_id).trigger("change");
                                        
                                        // Now load village
                                        loadVillageWithRetry(0);
                                    } else {
                                        loadCommuneWithRetry(retryCount + 1);
                                    }
                                }, 500);
                            }
                            
                            function loadVillageWithRetry(retryCount = 0) {
                                if (retryCount > 20) return;
                                
                                setTimeout(function() {
                                    var villageSelect = $("#"+addr_village+counter);
                                    
                                    if (villageSelect.find('option').length > 1) {
                                        villageSelect.val(data.village_id).trigger("change");
                                    } else {
                                        loadVillageWithRetry(retryCount + 1);
                                    }
                                }, 500);
                            }
                            
                            // Start the process
                            loadDistrictWithRetry();
                        }
                    });
                }
            });
        ...
        $("#company_phone_number, #phone_number, #phone_number2").keypress(function(event){
            if (!(event.charCode >= 48 && event.charCode <= 57)){ // 0-9 only
                event.preventDefault();
                return false;
            }
        });
