                                                ...
                                                @foreach ($cases as $case)
                                                    <tr class="border-bottom-success">
                                                        <td class="text-center">
                                                            <label
                                                                class="fw-bold">{{ $adata['cases']->firstItem() + $loop->iteration - 1 }}</label>
                                                        </td>
                                                        <td class="text-nowrap text-center">
                                                            <label
                                                                class="form-label m-0">កាលបរិច្ឆេទបណ្តឹង</label><br />
                                                            <span
                                                                class="text-danger fw-bold">[{{ date2Display($case->case_date) }}]</span><br /><br />
                                                            ...
                                                            {{--                                                        } --}}
                                                            {{--                                                    @endphp --}}
                                                            <label class="form-label blue fw-bold">*ការិយាល័យទី<span
                                                                    class="">{{ Num2Unicode(showCaseDomainID($case->id)) }}</span></label><br />
                                                        </td>
                                                        {{--                                                កម្មករដើមបណ្តឹង ក្រុមហ៊ុនចុងបណ្តឹង (កម្មករប្តឹងក្រុមហ៊ុន) --}}
                                                        @if ($case->case_type_id == 1)
                                                            <td class="text-center text-nowrap">
                                                                <label
                                                                    class="form-label fw-bold text-danger">{{ $case->caseDisputant->disputant->name }}</label>
                                                            </td>
                                                            <td class="text-center">
                                                                @if (!empty($case->company))
                                                                    <label
                                                                        class="form-label fw-bold blue">{{ $case->company->company_name_khmer }}</label>
                                                                    @if (!empty($case->company->company_name_latin))
                                                                        <br /><label
                                                                            class="form-label fw-bold blue">{{ $case->company->company_name_latin }}</label>
                                                                    @endif
                                                                @endif
                                                            </td>
                                                        ...
                                                        @elseif($case->case_type_id == 2)
                                                            ...
                                                            <td class="fw-bold text-danger">
                                                                {{ $case->caseDisputant->disputant->name }}</td>
                                                        @endif
                                                        ...
                                                        <td class="text-nowrap text-center">
                                                            ...
                                                            @endphp
                                                            <br>
                                                            <a class="btn btn-success-gradien custom fw-bold"
                                                                href="{{ url('cases/' . $case->id) }}"
                                                                title="មើលដំណើរការបណ្ដឹង"
