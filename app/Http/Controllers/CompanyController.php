...
class CompanyController extends Controller
{
    ...
    public $pageTitle = "បញ្ជីឈ្មោះរោងចក្រ សហគ្រាស";
    ...
    function getOrSearchEloquent1($province_id)
    {
        ...
        if (request('insp_status') && request('insp_status') > 0) {
            ...
            if (request('insp_status') <= 3) {
                ...
                $str = 0;
                $companys = $companys->whereDoesntHave(
                    "inspection",
    ...
    function getCompanyOrQuickSearchxxx($province_id)
    {
        ...
        $companys = DB::table("tbl_company_api AS c")
            ...
            ->select([
                ...
                "c.google_map_link",
                "c.encrypt_id",
                "c.company_name_khmer",
                "c.company_name_latin",
                "c.business_activity",
                "c.total_emp",
                "c.latest_total_emp",
                "c.latest_total_emp_female",
                "c.latest_total_emp_date",
                "c.latest_service",
                "c.latest_total_for",
                "c.latest_total_for_female",
                "c.total_emp_date",
                "c.company_register_number",
                "c.company_tin",
                "c.owner_khmer_name",
                "c.business_province",
                "c.business_district",
                "b.bus_khmer_name",
                "b.level AS business_activity_level",
                "b.group_id",
                "p.pro_khname AS province_name",
                "d.dis_khname AS district_name"
            ]);
    ...
    //     // Optional: page title
    //     $pagetitle = "កំណត់ត្រាគូវិវាទ";

    //     // If request from app
    //     if (request("json_opt") == 1) {
    //         return response()->json(['status' => 200, 'result' => compact('company')]);
    //     }

    //     // Pass variables directly to Blade
    //     return view('company.company_history_list', compact('company', 'pagetitle'));
    // }
