...
class Log6Controller extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'log620_agree_point.*'    => 'sometimes|string',
            'log620_solution.*'       => 'sometimes|string',
            'log621_disagree_point.*' => 'sometimes|string',
            'log621_solution.*'       => 'sometimes|string',
        ]);

        $validator->after(function ($validator) use ($request) {

            $hasValidAgree    = false;
            $hasValidDisagree = false;

            /** ========== 20. Agree + Solution ========== */
            foreach ($request->log620_agree_point ?? [] as $i => $agree) {
                $solution = $request->log620_solution[$i] ?? null;
                ...
                // half row → error
                if (($agree && !$solution) || (!$agree && $solution)) {
                    $validator->errors()->add(
                        "log620_agree_point.$i",
                        "ចំណុចព្រមព្រៀង និង ដំណោះស្រាយ ត្រូវបញ្ចូលទាំង ២"
                    );
                }
            }

            /** ========== 21. Disagree + Solution ========== */
            foreach ($request->log621_disagree_point ?? [] as $i => $disagree) {
                $solution = $request->log621_solution[$i] ?? null;

                if ($disagree && $solution) {
                    $hasValidDisagree = true;
                }

                // half row → error
                if (($disagree && !$solution) || (!$disagree && $solution)) {
                    $validator->errors()->add(
                        "log621_disagree_point.$i",
                        "ចំណុចមិនសះជា និង ដំណោះស្រាយ ត្រូវបញ្ចូលទាំង ២"
                    );
                }
            }

            /** ========== MUST HAVE AT LEAST ONE VALID PAIR ========== */
            if (!$hasValidAgree && !$hasValidDisagree) {
                $validator->errors()->add(
                    'log6_required',
                    'ត្រូវបញ្ចូលយ៉ាងហោចណាស់ ចំណុចព្រមព្រៀង ឬ ចំណុចមិនសះជា មួយ ជាមួយដំណោះស្រាយ'
                );
            }
        });
