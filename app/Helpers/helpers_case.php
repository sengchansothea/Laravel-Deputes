...
function showCollectivesInvitationBoth($case)
{
    ...
    if ($case->collectivesInvitationForConcilation->count() > 1) { //have collectives invitation for Conciliation
        ...
        return [
            "invitation_id1" => $invID[0],
            "invitation_file1" => $invFile[0],
            "info1" => $info[0],
            "export1" => "",
            "invitation_id2" => $invID[1],
            "invitation_file2" => $invFile[1],
            "info2" => $info[1],
            "export2" => ""
...
function showInvitationBothX($case)
{
    ...
    if ($case->invitationForConcilation->count() > 1) { //have invitation
        ...
        return [
            "invitation_id1" => $invitation_id[0],
            "invitation_file1" => $invitation_file[0],
            "info1" => $info[0],
            "export1" => $export[0],
            "invitation_id2" => $invitation_id[1],
            "invitation_file2" => $invitation_file[1],
            "info2" => $info[1],
            "export2" => $export[1]
        ];
    } else { //do not have invitation
...
function showCaseLog34($case): array
{
    ...
    if ($case->log34->isEmpty()) {
        ...
        if (!empty($case->lastOfficer())) {
            ...
            // Get only first valid invitation (avoid multiple buttons)
            $letterEmployee = $case->invitationAll()
                ->whereHas('invitationType', function ($q) {
                    $q->where('employee_or_company', 1);
                })
                ->where('invitation_type_id', 1)
                ->first();   // ✅ only first record

            $str .= "<div class='form-group col-sm-4'>";

            if ($letterEmployee) {
                $str .= "<a href='" . url('log34/create/' . $caseID . '/' . $letterEmployee->id) . "'
                         class='btn btn-outline-success form-control fw-bold'
                         title='Create'>
                         {$label}
                         </a>";
            } else {
                // fallback if no invitation
                $str .= "<a href='" . url('log34/create/' . $caseID . '/0') . "'
                         class='btn btn-outline-success form-control fw-bold'
                         title='Create'>
                         {$label}
                         </a>";
    ...
    // ===============================
    // CASE: Already Have Log34 (Edit + Export Mode)
    // ===============================
    else {

        $log = $case->log34->first(); // only first log
        ...
        if ($log && $log->detail34) {
            ...
            // Edit link
            $str .= "<div class='form-group col-sm-8'>
                        <a href='" . url('log34/' . $log34ID . '/edit') . "'
            ...
            // Export button
            $export = "<div class='form-group col-sm-4'>
                        <a class='btn btn-outline-info form-control fw-bold'
                           href='" . url('export/word/case/log34/' . $log34ID) . "'
                           title='Download'
                           target='_blank'>
                           ទាញយកកំណត់ហេតុ
                        </a>
...
function insertUpdateDisputantRepresentCompany($request, $log_id)
{ //បញ្ចូលឫកែប្រែ អ្នកតំណាងនិយោជក ឫអ្នកអមនិយោជក
    //    dd($request->all());
    $date_created = myDateTime();
    $case_id = $request->case_id;
